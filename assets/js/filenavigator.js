(function (angular) {
  ...
  angular.module('FileManagerApp').service('fileNavigator', [
    '$http', 'fileManagerConfig', 'item', function ($http, fileManagerConfig, Item) {
      ...
      FileNavigator.prototype.choose = function (item) {
        var self = this;
        if(item.model.chosen) {
          self.chosen[item.id] = item.model;
        } else {
          delete self.chosen[item.id];
        }

        console.log('self.chosen = ', self.chosen );
      }
